...
(function () {
    ...
    /* ===== Pechakucha Video — Reproductor Inline (YouTube facade) ===== */
    const videoLauncher = document.getElementById('videoLauncher');

    function loadInlineVideo() {
        if (!videoLauncher || videoLauncher.classList.contains('is-playing')) return;
        ...
        iframe.title = 'Pechakucha — Filoxenia';
        iframe.setAttribute('frameborder', '0');
        iframe.setAttribute('allow', 'accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share');
        iframe.setAttribute('allowfullscreen', '');

        videoLauncher.innerHTML = '';
        videoLauncher.appendChild(iframe);
        videoLauncher.classList.add('is-playing');
        videoLauncher.removeAttribute('role');
        videoLauncher.removeAttribute('tabindex');
        videoLauncher.removeAttribute('aria-label');
    }

    if (videoLauncher) {
        videoLauncher.addEventListener('click', loadInlineVideo);
        videoLauncher.addEventListener('keydown', (e) => {
